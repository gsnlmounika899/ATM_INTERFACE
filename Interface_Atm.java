...
class BankAccount {
	...
	public void register() {
		...
		System.out.println("\nRegistration is completed..kindly please login");
	}
			...
			if ( Username.equals(userName) ) {
				while ( !isLogin ) {
					...
					if ( Password.equals(password) ) {
						...
						System.out.println("\nYour Password is Incoorect!Please try again.");
					}
				}
			}
			else {
	...
	public void deposit() {
		...
		try {
			if ( amount <= 200000f ) {
				...
				System.out.println("\nOh!Sorry...The Limit is 200000.00");
			}
		...
		try {
			if ( balance >= amount ) {
				if ( amount <= 50000f ) {
					transactions++;
					balance -= amount;
					System.out.println("\nThe amount is Successfully Transfered to " + receipent);
